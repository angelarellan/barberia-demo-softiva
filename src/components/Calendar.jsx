    ...
    <div className="w-full rounded-2xl border border-white/10 bg-white/[0.03] p-4 xl:max-w-xs">
      <div className="flex items-center justify-between">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-full text-white/60 transition hover:bg-white/10 hover:text-white disabled:pointer-events-none disabled:opacity-0"
        >
          <ChevronLeft size={16} aria-hidden="true" />
        </button>
        <p aria-live="polite" className="text-sm font-semibold capitalize text-white">
      ...
      <div
        aria-hidden="true"
        className="mt-3 grid grid-cols-7 gap-1 text-center text-[11px] font-medium uppercase text-white/55"
      >
        ...
        {cells.map((cell, index) => {
          ...
          return (
            <button
              ...
              aria-label={formatDateLong(cell.iso)}
              className={`aspect-square rounded-lg text-sm font-medium transition ${
                isSelected
                  ? 'bg-gradient-to-br from-indigo-500 to-violet-600 text-white'
                  : isPast
                    ? 'cursor-not-allowed text-white/20'
                    : `text-white/70 hover:bg-white/10 ${isToday ? 'ring-1 ring-inset ring-indigo-400/50' : ''}`
              }`}
            >
              {cell.day}
            </button>
